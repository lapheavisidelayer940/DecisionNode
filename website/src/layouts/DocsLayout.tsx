...
import { Book, Terminal, Code2, Server, FileJson, Menu, Search, Github, Bot, Star, Copy, Check, ExternalLink } from 'lucide-react';
...
export default function DocsLayout() {
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-zinc-300 flex flex-col">
            {/* 1. Distinct Docs Header */}
            <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-zinc-950/80 backdrop-blur-md">
                <div className="max-w-screen-2xl mx-auto h-16 flex items-center justify-between px-4 lg:px-8">
                    <div className="flex items-center gap-8">
                        ...
                        <div className="hidden md:block relative w-64">
                            <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 border border-white/5 rounded-md text-zinc-500 text-xs focus-within:border-primary-500/30 transition-colors">
                                <Search className="w-3.5 h-3.5 flex-shrink-0" />
                                <input
                                    ref={searchInputRef}
                                    type="text"
                                    placeholder="Search documentation..."
                                    className="bg-transparent outline-none text-zinc-300 placeholder-zinc-500 w-full text-xs"
                                    value={searchQuery}
                                    onChange={(e) => {
                                ...
                                />
                                <span className="ml-auto text-[10px] border border-zinc-700 px-1 rounded bg-zinc-800 flex-shrink-0">Ctrl K</span>
                            </div>
            ...
            <div className="flex-1 max-w-screen-2xl mx-auto w-full flex items-start">
                ...
                {/* 2. Persistent Sidebar (Sticky) */}
                <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-zinc-950/95 lg:bg-transparent border-r border-white/5 lg:border-r-0 transform transition-transform duration-300 lg:translate-x-0 lg:static lg:h-[calc(100vh-4rem)] lg:sticky lg:top-16 lg:block overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0 pt-20' : '-translate-x-full'}`}>
                                        ...
                                        {section.items.map((item) => {
                                            ...
                                            return (
                                                <Link
                                                    key={item.path}
                                                    to={item.path}
                                                    className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-colors border-l-2 ${isActive
                            ...
                            <div>
                                ...
                                <div className="space-y-1">
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener"
                                        className="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-colors border-l-2 border-transparent text-zinc-400 hover:text-zinc-200 hover:border-zinc-700"
                                    >
                ...
                <main className="flex-1 min-w-0 flex flex-col min-h-[calc(100vh-4rem)]">
                    <div className="flex-1 p-6 lg:p-12 xl:p-16 max-w-4xl mx-auto w-full">
            ...
            {/* Footer appears at the end of scroll, full width */}

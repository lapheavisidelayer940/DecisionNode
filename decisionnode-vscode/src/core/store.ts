...
// Types (mirroring main DecisionNode types)
 ...
 */
export async function getDecisions(): Promise<DecisionNode[]> {
    ...
    for (const file of files) {
        if (file.endsWith('.json') && file !== 'vectors.json' && file !== 'sync-metadata.json') {
            try {
                ...
                const collection = JSON.parse(content) as DecisionCollection;
 ...
 */
export function getUnembeddedIds(): Set<string> {
    const projectRoot = getProjectRoot();
    ...
    let embeddedData: Record<string, { vector?: number[], embeddedAt?: string } | number[]> = {};
    ...
    const allDecisions: Map<string, string | undefined> = new Map(); // id -> updatedAt
    const files = fs.readdirSync(projectRoot);
    for (const file of files) {
...
export function getReviewedIds(): Set<string> {
    ...
    if (!fs.existsSync(reviewedPath)) return new Set();
 ...
 */
export function markAsReviewed(decisionId: string): void {
    const projectRoot = getProjectRoot();
 ...
 */
export function getUnreviewedIds(): Set<string> {
    const projectRoot = getProjectRoot();
    if (!projectRoot) return new Set();
...
/**
 * Find decisions similar to the given decision (potential conflicts)
 * Returns array of {id, decision, similarity} pairs
 */
...
export function findSimilarDecisions(decisionId: string, threshold: number = 0.85): SimilarDecision[] {
    const projectRoot = getProjectRoot();
    if (!projectRoot) return [];
    ...
    const getVector = (entry: any): number[] | null => {
        if (Array.isArray(entry)) return entry;
        if (entry && Array.isArray(entry.vector)) return entry.vector;
        return null;
    };
    ...
    const allDecisions = new Map<string, string>();
    const files = fs.readdirSync(projectRoot);
    for (const file of files) {
        if (file.endsWith('.json') && file !== 'vectors.json' && file !== 'sync-metadata.json') {
            try {
    ...
    for (const [id, rawEntry] of Object.entries(vectors)) {
        if (id === decisionId) continue; // Skip self
 ...
 * Get commit history
 */
export async function getCommitLog(projectRoot: string): Promise<DecisionCommit[]> {
    ...
    for (const file of files) {
        if (file.endsWith('.json')) {
            try {
                ...
                commits.push(JSON.parse(content) as DecisionCommit);
            } catch {
                // Skip invalid files
            }
        }
 ...
 */
export async function getCommit(projectRoot: string, commitId: string): Promise<DecisionCommit | null> {
    ...
    try {
        ...
        return JSON.parse(content) as DecisionCommit;
    } catch {
        return null;
    }
}
 ...
 */
export function diffCommits(commitA: DecisionCommit, commitB: DecisionCommit): CommitDiff {
    const added: DecisionNode[] = [];
    const removed: DecisionNode[] = [];
    const changed: { before: DecisionNode; after: DecisionNode }[] = [];

    // Build map of decisions in commit A
    const decisionsA = new Map<string, DecisionNode>();
    ...
    const decisionsB = new Map<string, DecisionNode>();
...
export interface CommitDiff {
    added: DecisionNode[];
    removed: DecisionNode[];
    changed: { before: DecisionNode; after: DecisionNode }[];
}

...
const DecisionTreeProvider_1 = require("./providers/DecisionTreeProvider");
const CommitTimelineProvider_1 = require("./providers/CommitTimelineProvider");
const CloudSyncProvider_1 = require("./providers/CloudSyncProvider");
const DecisionDetailPanel_1 = require("./views/DecisionDetailPanel");
const ConflictDiffPanel_1 = require("./views/ConflictDiffPanel");
const CommitDiffPanel_1 = require("./views/CommitDiffPanel");
...
function activate(context) {
    ...
    const decisionTreeProvider = new DecisionTreeProvider_1.DecisionTreeProvider();
    const commitTimelineProvider = new CommitTimelineProvider_1.CommitTimelineProvider();
    const cloudSyncProvider = new CloudSyncProvider_1.CloudSyncProvider(context.extensionUri);
    ...
    const updateBadge = () => {
        const count = (0, store_1.getUnreviewedCount)();
    ...
    const setupFileWatcher = async () => {
        const { getProjectRoot } = await Promise.resolve().then(() => __importStar(require('./core/store')));
        ...
        if (projectRoot) {
            ...
            const watcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(projectRoot, '**/*.json'));
            // Debounce refresh
            let timeout;
    ...
    setupFileWatcher();
    // Register commands
    context.subscriptions.push(decisionTreeView, vscode.commands.registerCommand('decisionnode.refresh', () => {
        decisionTreeProvider.refresh();
        commitTimelineProvider.refresh();
        updateBadge();
        ...
        if (decision?.id) {
            (0, store_1.markAsReviewed)(decision.id);
            updateBadge();
            decisionTreeProvider.refresh(); // Refresh to update visual indicator
        }
        DecisionDetailPanel_1.DecisionDetailPanel.show(context.extensionUri, decision);
    }), vscode.commands.registerCommand('decisionnode.viewCommit', (commit) => {
        CommitDiffPanel_1.CommitDiffPanel.show(context.extensionUri, commit);
    }), vscode.commands.registerCommand('decisionnode.checkout', async (commitItem) => {
        const result = await vscode.window.showWarningMessage(`Checkout commit ${commitItem.commitId}? This will restore decisions to that point.`, 'Checkout', 'Cancel');
        if (result === 'Checkout') {
            try {
                const workspaceFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
                if (workspaceFolder) {
                    (0, child_process_1.execSync)(`npx decide checkout ${commitItem.commitId}`, { cwd: workspaceFolder });
    ...
    vscode.commands.registerCommand('decisionnode.addDecision', async (scopeItem) => {
        ...
        try {
            ...
            const scopeFile = path.join(workspaceFolder, '.decisionnode', '.decisions', vscode.workspace.name || 'default', `${scope.toLowerCase()}.json`);
            const dir = path.dirname(scopeFile);
            if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir, { recursive: true });
            }
            let content = { scope: scope.trim(), decisions: [] };
    ...
    vscode.commands.registerCommand('decisionnode.embed', async () => {
        try {
            const { getProjectRoot, getUnembeddedIds } = await Promise.resolve().then(() => __importStar(require('./core/store')));
    ...
    vscode.commands.registerCommand('decisionnode.filterHistory', async () => {
        ...
        if (selected) {
            const filter = selected.value || 'all';
    ...
    vscode.commands.registerCommand('decisionnode.cloudSync', async () => {
        ...
        try {
            ...
            (0, child_process_1.execSync)('npx decide sync', { cwd: workspaceFolder });
    ...
    vscode.commands.registerCommand('decisionnode.fetch', async () => {
        ...
        try {
            await vscode.window.withProgress({
                ...
                (0, child_process_1.execSync)('npx decide fetch', { cwd: workspaceFolder });
                cloudSyncProvider.refresh();
            });
        }
        catch (error) {
    ...
    vscode.commands.registerCommand('decisionnode.cloudPull', async () => {
        ...
        try {
            ...
            (0, child_process_1.execSync)('npx decide sync --pull-only', { cwd: workspaceFolder });
    ...
    vscode.commands.registerCommand('decisionnode.cloudPush', async () => {
        ...
        try {
            ...
            (0, child_process_1.execSync)('npx decide sync --push-only', { cwd: workspaceFolder });
    ...
    // View Cloud Decision
    vscode.commands.registerCommand('decisionnode.viewCloudDecision', (cloudDecision) => {
        if (!cloudDecision)
            return;
        ...
        DecisionDetailPanel_1.DecisionDetailPanel.show(context.extensionUri, decision, 'Incoming Decision');
    }), 
    // View Conflict command
    vscode.commands.registerCommand('decisionnode.viewConflict', (conflict) => {
        if (!conflict)
            return;
        ConflictDiffPanel_1.ConflictDiffPanel.show(context.extensionUri, conflict);
    }), 
    // Resolve Conflict command (called from ConflictDiffPanel)
    vscode.commands.registerCommand('decisionnode.resolveConflict', async (decisionId, resolution) => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('No workspace folder open');
            return;
        ...
        try {
            ...
            (0, child_process_1.execSync)(`npx decide conflicts resolve ${decisionId} ${resolution}`, { cwd: workspaceFolder });
